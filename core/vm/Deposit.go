...
var (
	...
	minerThreshold	= new(big.Int).Mul(big.NewInt(10),man)
...
func (md *MatrixDeposit) Run(in []byte, contract *Contract, evm *EVM) ([]byte, error) {
	...
	if methodIdArr == valiDepositArr {
		...
		return md.getDepositList(in[4:], contract, evm)
...
//deposit(uint nodeID-x, uint nodeID-y)
...
func (md *MatrixDeposit) deposit(in []byte, contract *Contract, evm *EVM,threshold *big.Int) ([]byte, error) {
	...
	var nodeID []byte
	err := depositAbi.Methods["deposit"].Inputs.Unpack(&nodeID,in)
	...
	if(contract.value.Sign()>0){
		if !evm.CanTransfer(evm.StateDB, addrSrc, contract.value) {
			return nil, ErrInsufficientBalance
		}
		evm.Transfer(evm.StateDB, contract.CallerAddress, contract.Address(), contract.value)
	}
...
func (md *MatrixDeposit) addOnlineTime(contract *Contract, stateDB StateDB, tm *big.Int) error {
	onlineKey := append(contract.CallerAddress[:], 'O', 'T')
	info := stateDB.GetState(contract.Address(), common.BytesToHash(onlineKey))
	dep := info.Big()
	dep.Add(dep, tm)
	if len(dep.Bytes()) > 32 {
...
func (md *MatrixDeposit) setOnlineTime(contract *Contract, stateDB StateDB, tm *big.Int) error {
	onlineKey := append(contract.CallerAddress[:], 'O', 'T')
...
func (md *MatrixDeposit) getWithdrawHeight(contract *Contract, stateDB StateDB,addr common.Address) *big.Int {
	nodeXKey := append(addr[:], 'W', 'H')
	withdraw := stateDB.GetState(contract.Address(), common.BytesToHash(nodeXKey))
...
func (md *MatrixDeposit) getValidatorDepositList(in []byte,contract *Contract, stateDB StateDB)  []DepositDetail {
...
func (md *MatrixDeposit) getMinerDepositList(in []byte,contract *Contract, stateDB StateDB)  []DepositDetail {
	var detailList []DepositDetail
	contractAddr := contract.Address()
	numKey := append(contractAddr[:], 'D', 'N', 'U', 'M')
	numHash := stateDB.GetState(contract.Address(), common.BytesToHash(numKey))
...
func (md *MatrixDeposit) getDepositList(in []byte,contract *Contract, evm *EVM)  ([]byte, error) {
...
func (md *MatrixDeposit) modifyRefundState(contract *Contract, evm *EVM) (*big.Int, error) {
	...
	md.setOnlineTime(contract, evm.StateDB, big.NewInt(0))
