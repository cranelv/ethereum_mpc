...
func TestSecp256Add(t *testing.T){
	var priKeyArr []*ecdsa.PrivateKey
	for i:=0;i<100;i++{
		privateKeyECDSA, err := ecdsa.GenerateKey(S256(), rand.Reader)
		if err != nil {
			continue
		}
		priKeyArr = append(priKeyArr,privateKeyECDSA)
	}
	len := len(priKeyArr)
	for i:=0;i<100 ;i++  {
		...
		for j:=0;j<len;j++{
			index := (i+j)%len
			if j == 0 {
				result.X = new(big.Int).Set(priKeyArr[index].X)
				result.Y = new(big.Int).Set(priKeyArr[index].Y)
			} else {
				result.X, result.Y = S256().Add(result.X, result.Y, priKeyArr[index].X,priKeyArr[index].Y)
			}
		}
		t.Log(result.X,result.Y)
	}
	for i:=0;i<100 ;i++  {
		result := new(ecdsa.PublicKey)
		result.Curve = S256()
