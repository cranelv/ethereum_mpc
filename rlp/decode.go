...
//decode tuple item
//first encode item1, encode item2 ... encode itemn
func DecodeTupleItem(r io.Reader,index int, val interface{}) error {
	// TODO: this could use a Stream from a pool.
	return NewStream(r, 0).DecodeItem(index,val)
}
func DecodeTupleCount(r io.Reader) (int,error){
	return NewStream(r, 0).DecodeTupleCount()
}
...
func (s *Stream) DecodeItem(index int,val interface{}) error {
	if val == nil {
...
func (s *Stream) DecodeTupleCount() (int,error) {
	var i int
	for i=0;;i++ {
		_, err := s.Raw()
		if err != nil {
