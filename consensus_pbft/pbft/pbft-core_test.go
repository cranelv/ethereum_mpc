...
func TestMaliciousPrePrepare(t *testing.T) {
	...
	digest,_ := createPbftReqBatch(1, 1).Marshal()
	pbftMsg := &message.PrePrepare{
		View:           0,
		SequenceNumber: 1,
		BatchDigest:    pbftTypes.MessageDigest(digest),
...
func TestWrongReplicaID(t *testing.T) {
	...
	hash,_ := reqBatch.Marshal()
	pbftMsg := &message.PrePrepare{
		View:           0,
		SequenceNumber: 1,
		BatchDigest:    pbftTypes.MessageDigest(hash),
...
func TestIncompletePayload(t *testing.T) {
	...
	broadcaster := pbftTypes.ReplicaID(generateBroadcaster(uint32(instance.replicaCount)))

	checkMsg := func(msg message.MessageInterface, errMsg string, args ...interface{}) {
	...
	checkMsg(&message.Request{}, "Expected to reject empty message")
	checkMsg(&message.PrePrepare{ReplicaId: broadcaster}, "Expected to reject empty pre-prepare")
}

func TestNetwork(t *testing.T) {
	log.InitLog(5)
	validatorCount := uint32(7)
	net := makePBFTNetwork(validatorCount, nil)

	reqBatch := createPbftReqBatch(1, pbftTypes.ReplicaID(generateBroadcaster(validatorCount)))
	...
	for _, pep := range net.pbftEndpoints {
		...
		hash,_ := (*reqBatch)[0].Marshal()
		if !reflect.DeepEqual(pep.sc.lastExecution, hash) {
			t.Errorf("Instance %d executed wrong transaction, %x should be %x",
				pep.id, pep.sc.lastExecution, hash)
		}
	}
}
...
func TestCheckpoint(t *testing.T) {
	...
	config := loadConfig()
	config.K = 2
	config.LogMultiplier = 2
	net := makePBFTNetwork(validatorCount, config)
	defer net.stop()

	execReqBatch := func(tag int64) {
		net.pbftEndpoints[0].manager.Queue() <- createPbftReqBatch(tag, pbftTypes.ReplicaID(generateBroadcaster(validatorCount)))
		net.process()
	}
...
func TestLostPrePrepare(t *testing.T) {
	...
	net.pbftEndpoints[0].manager.Queue() <- createPbftReqBatch(1, pbftTypes.ReplicaID(generateBroadcaster(validatorCount)))

	// clear all messages sent by primary
	msg := <-net.msgs
	var prePrep message.MessageInterface
	err := message.UnmarshalMsg(msg.msg,&prePrep)
	if err != nil {
...
func TestInconsistentPrePrepare(t *testing.T) {
	...
	makePP := func(tag int64) *message.PrePrepare {
		reqBatch := createPbftReqBatch(tag, pbftTypes.ReplicaID(generateBroadcaster(validatorCount)))
		hash,_ := reqBatch.Marshal()
	...
	net.pbftEndpoints[0].manager.Queue() <- makePP(1).RequestBatch
...
func TestViewChangeWatermarksMovement(t *testing.T) {
	...
	instance := newPbftCore(0, loadConfig(), &omniProto{
		...
		skipToImpl: func(s uint64, id []byte, replicas []pbftTypes.ReplicaID) {
	...
	vset := make([]*message.ViewChange, 3)

	// Replica 0 sent checkpoints for 10
	vset[0] = &message.ViewChange{
		H: 5,
		Cset: []*message.ViewChange_C{
			{
				SequenceNumber: 10,
				Id:             "ten",
			},
	...
	xset := make(map[uint64]pbftTypes.MessageDigest)
	xset[11] = ""

	instance.newViewStore[1] = &message.NewView{
		View:      1,
		Vset:      vset,
		Xset:      xset,
		ReplicaId: 1,
...
func TestInconsistentDataViewChange(t *testing.T) {
	...
	makePP := func(tag int64) *message.PrePrepare {
		...
		preprep := &message.PrePrepare{
			View:           0,
			SequenceNumber: 1,
			BatchDigest:    message.Digest(reqBatch),
			RequestBatch:   reqBatch,
			ReplicaId:      0,
		}
		return preprep
	...
	net.pbftEndpoints[0].manager.Queue() <- makePP(0).RequestBatch
...
func TestViewChangeWithStateTransfer(t *testing.T) {
	...
	broadcaster := pbftTypes.ReplicaID(generateBroadcaster(validatorCount))
	...
	for i := int64(1); i <= 3; i++ {
		net.pbftEndpoints[0].manager.Queue() <- makePP(i).RequestBatch
	...
	net.pbftEndpoints[1].manager.Queue() <- makePP(5).RequestBatch
	err = net.process()
	if err != nil {
		t.Fatalf("Processing failed: %s", err)
	}
...
func TestNewViewTimeout(t *testing.T) {
	...
	validatorCount := uint32(4)
	config := loadConfig()
	config.Request = 400 * time.Millisecond
	config.Viewchange = 800 * time.Millisecond
	net := makePBFTNetwork(validatorCount, config)
	defer net.stop()
...
func TestViewChangeUpdateSeqNo(t *testing.T) {
	...
	validatorCount := uint32(4)
	config.Request = 400 * time.Millisecond
	config.Viewchange = 400 * time.Millisecond
	net := makePBFTNetwork(validatorCount, config)
...
func TestWitnessCheckpointOutOfBounds(t *testing.T) {
	...
	events.SendEvent(instance, &message.Checkpoint{
		SequenceNumber: 6,
		ReplicaId:      0,
	})
...
func TestWitnessFallBehindMissingPrePrepare(t *testing.T) {
	...
	events.SendEvent(instance, &message.Commit{
		SequenceNumber: 2,
		ReplicaId:      0,
	})
...
func TestPbftF0(t *testing.T) {
	...
	for _, pep := range net.pbftEndpoints {
		...
		if !reflect.DeepEqual(pep.sc.lastExecution, message.Digest((*reqBatch)[0])) {
			t.Errorf("Instance %d executed wrong transaction, %x should be %x",
				pep.id, pep.sc.lastExecution, message.Digest((*reqBatch)[0]))
		}
	}
}
...
func TestRequestTimerDuringViewChange(t *testing.T) {
	mock := &omniProto{
		...
		verifyImpl: func(senderID pbftTypes.ReplicaID, signature []byte, message []byte) error { return nil },
...
func TestReplicaCrash1(t *testing.T) {
	...
	for id := 0; id < 2; id++ {
		...
		pe.pbft = newPbftCore(pbftTypes.ReplicaID(id), loadConfig(), pe.sc, events.NewTimerFactoryImpl(pe.manager))
	...
	net.pbftEndpoints[0].manager.Queue() <- createPbftReqBatch(2, pbftTypes.ReplicaID(generateBroadcaster(validatorCount)))
	net.pbftEndpoints[0].manager.Queue() <- createPbftReqBatch(3, pbftTypes.ReplicaID(generateBroadcaster(validatorCount)))
	net.process()
...
func TestReplicaCrash2(t *testing.T) {
	validatorCount := uint32(4)
	config := loadConfig()
	config.Request = 800 * time.Millisecond
	config.Viewchange = 800 * time.Millisecond
	config.K = 2
	config.LogMultiplier = 2
	net := makePBFTNetwork(validatorCount, config)
	...
	net.filterFn = func(src int, dst int, msg []byte) []byte {
		...
		var pm message.MessageInterface
		err := message.UnmarshalMsg(msg, &pm)
		if err != nil {
			t.Fatal(err)
		}
		// filter commits to all but 1
		commit := pm.(*message.Commit)
	...
	net.pbftEndpoints[primary].manager.Queue() <- createPbftReqBatch(2, pbftTypes.ReplicaID(generateBroadcaster(validatorCount)))
	net.pbftEndpoints[primary].manager.Queue() <- createPbftReqBatch(3, pbftTypes.ReplicaID(generateBroadcaster(validatorCount)))
	net.pbftEndpoints[primary].manager.Queue() <- createPbftReqBatch(4, pbftTypes.ReplicaID(generateBroadcaster(validatorCount)))
	go net.processContinually()
...
func TestReplicaCrash3(t *testing.T) {
	...
	for i := int64(1); i <= 8; i++ {
		net.pbftEndpoints[0].manager.Queue() <- createPbftReqBatch(i, pbftTypes.ReplicaID(generateBroadcaster(validatorCount)))
	}
	...
	for id := 0; id < 2; id++ {
		...
		config.K = 2
		pe.pbft.close()
		pe.pbft = newPbftCore(pbftTypes.ReplicaID(id), config, pe.sc, events.NewTimerFactoryImpl(pe.manager))
		pe.manager.SetReceiver(pe.pbft)
		pe.pbft.N = 4
		pe.pbft.f = (4 - 1) / 3
	...
	net.pbftEndpoints[0].manager.Queue() <- createPbftReqBatch(9, pbftTypes.ReplicaID(generateBroadcaster(validatorCount)))
	net.process()
...
func TestReplicaPersistQSet(t *testing.T) {
	...
	reqBatch := createPbftReqBatch(1, 0)
	events.SendEvent(p, &message.PrePrepare{
		View:           0,
		SequenceNumber: 1,
		BatchDigest:    message.Digest(reqBatch),
		RequestBatch:   reqBatch,
		ReplicaId:      pbftTypes.ReplicaID(0),
	})
	p.close()

	p = newPbftCore(1, loadConfig(), stack, &inertTimerFactory{})
	if !p.prePrepared(message.Digest(reqBatch), 0, 1) {
...
func TestReplicaPersistDelete(t *testing.T) {
	...
	p := newPbftCore(1, loadConfig(), stack, &inertTimerFactory{})
	p.reqBatchStore["a"] = &message.RequestBatch{}
...
func TestNetworkNullRequests(t *testing.T) {
	validatorCount := uint32(4)
	config := loadConfig()
	config.Nullrequest = 200 * time.Millisecond
	config.Request = 400 * time.Millisecond
	net := makePBFTNetwork(validatorCount, config)
	defer net.stop()
...
func TestNetworkNullRequestMissing(t *testing.T) {
	...
	config := loadConfig()
	config.Nullrequest = 200 * time.Millisecond
	config.Request = 500 * time.Millisecond
	net := makePBFTNetwork(validatorCount, config)
	defer net.stop()
...
func TestViewChangeCannotExecuteToCheckpoint(t *testing.T) {
	...
	for i := instance.lastExec; i < newViewBaseSeqNo; i++ {
		commit := &message.Commit{View: 0, SequenceNumber: i}
		prepare := &message.Prepare{View: 0, SequenceNumber: i}
		instance.certStore[msgID{v: 0, n: i}] = &msgCert{
			digest:     "", // null request
			prePrepare: &message.PrePrepare{View: 0, SequenceNumber: i},
			prepare:    []*message.Prepare{prepare, prepare, prepare},
			commit:     []*message.Commit{commit, commit, commit},
		}
	}
...
func TestStateNetworkMovesOnDuringSlowStateTransfer(t *testing.T) {
	...
	for i := uint64(0); i < 3; i++ {
		events.SendEvent(instance, &message.Checkpoint{
			...
			ReplicaId:      pbftTypes.ReplicaID(i),
...
func TestCheckpointDiffersFromWeakCert(t *testing.T) {
	...
	badChkpt := &message.Checkpoint{
		SequenceNumber: 10,
		Id:             "WRONG",
		ReplicaId:      pbftTypes.ReplicaID(3),
	}
