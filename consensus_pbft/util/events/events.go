...
func (t *threaded) Halt() {
	select {
	case <-t.exit:
		singletons.Log.Warn("Attempted to halt a threaded object twice")
	default:
		close(t.exit)
	}
}
...
func (em *managerImpl) eventLoop() {
	for {
		...
		case next := <-em.events:
			em.Inject(next)
		case <-em.exit:
			singletons.Log.Debug("eventLoop told to exit")
			return
		}
	}
}
...
func (et *timerImpl) loop() {
	...
	for {
		...
		case start := <-et.startChan:
			if et.timerChan != nil {
				if start.hard {
					singletons.Log.Debug("Resetting a running timer")
				} else {
					continue
				}
			}
			singletons.Log.Debug("Starting timer")
			et.timerChan = time.After(start.duration)
			if eventDestChan != nil {
				singletons.Log.Debug("Timer cleared pending event")
			}
			event = start.event
			eventDestChan = nil
		case <-et.stopChan:
			if et.timerChan == nil && eventDestChan == nil {
				singletons.Log.Debug("Attempting to stop an unfired idle timer")
			}
			et.timerChan = nil
			singletons.Log.Debug("Stopping timer")
			if eventDestChan != nil {
				singletons.Log.Debug("Timer cleared pending event")
			}
			eventDestChan = nil
			event = nil
		case <-et.timerChan:
			singletons.Log.Debug("Event timer fired")
			et.timerChan = nil
			eventDestChan = et.manager.Queue()
		case eventDestChan <- event:
			singletons.Log.Debug("Timer event delivered")
			eventDestChan = nil
		case <-et.exit:
			singletons.Log.Debug("Halting timer")
			return
		}
	}
}
