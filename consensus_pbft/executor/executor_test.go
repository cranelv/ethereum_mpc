...
type mockConsumer struct {
	...
	CommittedImpl    func(tag interface{},  state  *message.StateInfo) // Called whenever Commit completes
	RolledBackImpl   func(tag interface{})                            // Called whenever a Rollback completes
	StateUpdatedImpl func(tag interface{},  state  *message.StateInfo) // Called when state transfer completes, if target is nil, this indicates a failure and a new target should be supplied
}
...
func (mock *mockConsumer) Committed(tag interface{},  state  *message.StateInfo) {
	if mock.CommittedImpl != nil {
		mock.CommittedImpl(tag, state)
	}
}
...
func (mock *mockConsumer) StateUpdated(tag interface{},  state  *message.StateInfo) {
	if mock.StateUpdatedImpl != nil {
		mock.StateUpdatedImpl(tag, state)
	}
}
...
func (mock *mockRawExecutor) BeginTaskBatch(id interface{}) error {
...
func (mock *mockRawExecutor) ExecTasks(id interface{}, txs []*message.Task) ([]*message.Result, error) {
	if mock.curBatch != id {
...
func (mock *mockRawExecutor) CommitTaskBatch(id interface{}, meta []byte) (*message.StateInfo, error) {
	if mock.curBatch != id {
...
func (mock *mockRawExecutor) RollbackTaskBatch(id interface{}) error {
	if mock.curBatch == nil {
...
func (mock *mockRawExecutor) PreviewCommitTaskBatch(id interface{}, meta []byte) ([]byte, error) {
	if mock.curBatch != nil {
...
func (mock *mockRawExecutor) GetStateInfo() *message.StateInfo{
//	return nil
	return &message.StateInfo{
		Hash: pbftTypes.MessageDigest(fmt.Sprintf("%d", mock.commitCount)),
		Number: mock.commitCount,
	}
//		Number:			big.NewInt(int64(mock.commitCount)),
//		ParentHash:		[]byte(fmt.Sprintf("%d", mock.commitCount-1))
...
type mockStateTransfer struct {
	StartImpl        func()
	StopImpl         func()
	SyncToTargetImpl func(blockNumber uint64, digest pbftTypes.MessageDigest, peerIDs []*pbftTypes.PeerID) (error, bool)
}

func (mock *mockStateTransfer) Start() {}
func (mock *mockStateTransfer) Stop()  {}

func (mock *mockStateTransfer) SyncToTarget(blockNumber uint64, digest pbftTypes.MessageDigest, peerIDs []*pbftTypes.PeerID) (error, bool) {
	if mock.SyncToTargetImpl != nil {
		return mock.SyncToTargetImpl(blockNumber, digest, peerIDs)
	}
	return nil, false
}
...
func TestNormalExecutes(t *testing.T) {
	...
	id := struct{}{}
	testTxs := []*message.Task{&message.Task{}, &message.Task{}, &message.Task{}}
	...
	committed := false
	mc.CommittedImpl = func(tag interface{}, info *message.StateInfo) {
		if tag != id {
...
func TestNormalStateTransfer(t *testing.T) {
	...
	blockHash := pbftTypes.MessageDigest("BlockHash")

	stateUpdated := false
	mc.StateUpdatedImpl = func(tag interface{}, info *message.StateInfo) {
		if id != tag {
	...
	count := 0
	mst.SyncToTargetImpl = func(bn uint64, bh pbftTypes.MessageDigest, ps []*pbftTypes.PeerID) (error, bool) {
		count++
		if count <= 10 {
...
//	co.UpdateState(id, &message.StateInfo{Height: blockNumber + 1, CurrentBlockHash: blockHash}, nil)
	co.UpdateState(id, &message.StateInfo{Number: blockNumber + 1, Hash: blockHash}, nil)
	mev.process()
...
func TestFailingStateTransfer(t *testing.T) {
	...
	blockHash1 := pbftTypes.MessageDigest("BlockHash1")
	blockNumber2 := uint64(2)
	blockHash2 := pbftTypes.MessageDigest("BlockHash2")
	...
	count := 0
	mst.SyncToTargetImpl = func(bn uint64, digest pbftTypes.MessageDigest, peerIDs []*pbftTypes.PeerID) (error, bool) {
		count++
		if count <= 10 {
			return fmt.Errorf("Transient state transfer error"), true
		}
	...
	co.UpdateState(id, &message.StateInfo{Number: blockNumber1 + 1, Hash: blockHash1}, nil)
//	co.UpdateState(id, &pb.BlockchainInfo{Height: blockNumber1 + 1, CurrentBlockHash: blockHash1}, nil)
	...
	co.UpdateState(id, &message.StateInfo{Number: blockNumber2 + 1, Hash: blockHash2}, nil)
